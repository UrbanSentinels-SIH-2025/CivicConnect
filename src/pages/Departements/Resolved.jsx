    ...
    const fetchData = async () => {
      try {
        ...
        const resolvedIssues = issuesData.filter(
          (issue) => issue.progress?.resolved?.completed
        );
        
        setIssues(resolvedIssues);
        setFilteredIssues(resolvedIssues);
        generateThumbnails(resolvedIssues);
      } catch (error) {
    ...
    <div className="p-4 md:p-6 min-h-screen bg-gradient-to-b from-blue-50 to-blue-200">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-6 md:mb-8">
          ...
          <p className="text-lg md:text-xl text-gray-600">Resolved Issues</p>
          <div className="mt-3 md:mt-4 bg-white inline-flex rounded-full px-3 md:px-4 py-1 md:py-2 shadow">
            <span className="font-semibold text-gray-700 text-sm md:text-base">
              Total Resolved Issues: {filteredIssues.length}
            </span>
          </div>
        </div>
        ...
        {filteredIssues.length > 0 ? (
          <div className="space-y-4 md:space-y-6">
            {filteredIssues.map((issue) => (
              <div
                key={issue._id}
                className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border-l-4 border-green-500"
              >
                    ...
                    <div className="flex flex-col h-full justify-between">
                      <div>
                        ...
                        <div className="mb-4 md:mb-6">
                          ...
                          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 md:gap-4 text-xs md:text-sm">
                            ...
                            <div className="flex items-center">
                              <div className="w-2 h-2 md:w-3 md:h-3 bg-green-600 rounded-full mr-2"></div>
                              <span className="font-semibold">Resolved: {new Date(issue.progress.resolved.date).toLocaleDateString()}</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="pt-3 md:pt-4 border-t border-gray-100">
                        <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
                          <div className="flex-1 px-4 md:px-6 py-2 md:py-3 bg-gray-100 text-gray-600 font-semibold rounded-lg text-center text-sm md:text-base">
                            Issue Resolved
                          </div>
                          <NavLink
                            to={`/department/${departmentName}/issue-details/${issue._id}`}
                            className="flex-1 px-4 md:px-6 py-2 md:py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg transition-all duration-300 text-center shadow-md hover:shadow-lg text-sm md:text-base"
                          >
                            View Details
                          </NavLink>
                        </div>
          ...
          <div className="text-center py-8 md:py-12 bg-white rounded-xl shadow-lg">
            ...
            <p className="text-lg md:text-xl text-gray-600 mb-2">No resolved issues found</p>
            <p className="text-gray-500 text-sm md:text-base">All issues are either pending or in progress.</p>
          </div>
        )}
      </div>
    </div>
